...
## Data Setup
Here we clean the data down to our selected sample and scale the predictors. 
```{r data,include=F}
...
```{r}
set.seed(123)
cboot <- coralboot(n.boot=1000,verbose=F)
cboot$mse.min %>% mean()
cboot$mse.1se %>% mean()
...
```{r}
test <- lm(U238 ~ OmegaA + pH + TAlk + TCO2 + Temperature, data=coral.df)
summary(test)
```
